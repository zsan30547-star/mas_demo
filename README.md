...
> ⚠️ **使用前须知**
>
> 1. **API Key**：本项目依赖大模型 API（DeepSeek / Qwen / Gemini / Claude），使用前请在管理后台「模型配置」中添加对应的 API Key，或在 `ai_engine/.env` 中配置。
> 2. **原型阶段**：当前项目处于原型开发阶段，部分功能仍在优化中，可能存在未完善的地方，欢迎提交 Issue 或 PR。
> 3. **数据安全**：内置的数据库初始化脚本包含演示数据，请勿直接用于生产环境。
